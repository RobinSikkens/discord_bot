""" Registers all commands and presents them in a command dict. """
...
class Register(object):
    """ General decorator for registering commands.

    Must be overriden to specify target dict.
    """

    target_dict = {}

    def __init__(self, name):
        """ Register a command's name and aliases. """
        self.name = name

    def __call__(self, func):
        """ Register callable under name. """

        # If name is iterable it has aliases, all names are registered.
        ...
        LOGGER.debug(f"Registered {self.name}.")
        return func


COMMAND_DICT = {}
""" Maps commands to callables. """

COMMAND_CATEGORIES = defaultdict(list)
""" Maps categories to lists of (name, aliases, callable) for the generation of a help message. """


class Command(Register):
    """ Decorator to turn functions into bot commands. """
